...
export default function UserPointHis() {
  ...
  const handlePoinData = e => {
    ...
    const value = type.includes("po_point") ? e.target.value.replace(/[^0-9]/, "") : e.target.value;
  ...
  const checkAll = e => {
    if (e.target.checked) setCheckedList(resData.pointInfo.map(el => el.po_id));
    else setCheckedList([]);
  };
  ...
  return (
    <>
      ...
      <CurrentBox btns={["del", "down"]} tit="포인트 지급/사용 내역" {...btnEvent} />
        ...
        <div className="table_wrap line part">
          <table className="table">
            ...
            <thead>
              <tr>
                <th className="check">
                  <CheckBox for="wr_all" id="wr_all" name="wr_all" checked={resData?.pointInfo.length === checkedList.length} onClick={checkAll} />
                </th>
              ...
              <tr className="write_row">
                ...
                <td className="date">
                  <div className="input_ty02">
                    <input type="text" placeholder="YYYY-MM-DD" value={addPointData.po_datetime} data-type="po_datetime" onChange={handlePoinData} />
                  </div>
                </td>
                <td>
                  <div className="input_ty02">
                    <input type="text" placeholder="직접입력" value={addPointData.po_content} data-type="po_content" onChange={handlePoinData} />
                  </div>
                </td>
                <td>
                  <div className="input_ty02">
                    <input type="text" placeholder="직접입력" value={addPointData.po_point} data-type="po_point" onChange={handlePoinData} />
                  </div>
                </td>
                <td>
                  <div className="input_ty02">
                ...
                <td>
                  <div className="input_ty02">
                    <input type="text" placeholder="직접입력" value={addPointData.po_memo} data-type="po_memo" onChange={handlePoinData} />
                  </div>
                </td>
              </tr>
              {resData?.pointInfo.map((el, idx) => {
                return <PointItem key={idx} data={el} checkedList={checkedList} setCheckedList={setCheckedList} />;
              })}
...
function PointItem({ data, checkedList, setCheckedList }) {
  ...
  return (
    <tr>
      <td className="check">
        <CheckBox for={data.po_id} id={data.po_id} name={data.po_id} checked={checkedList.includes(data.po_id)} onClick={checkItem} />
      </td>
