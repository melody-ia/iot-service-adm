      ...
      <div className="point box_ty01 table_type table_comm accumulated">
        ...
        <div className="table_wrap line">
          <table className="table" id="table">
            ...
            <thead>
              <tr>
                <th className="check">{/* <CheckBox for="wr_all" id="wr_all" name="wr_all" /> */}</th>
                ...
                resData?.map((el, idx) => {
                  return (
                    <JoinPoint
                      ...
                      setEditItemList={setEditItemList}
                    />
                  );
                })}
              {/* <PromotionPoint /> */}
...
function AddPoint({ isChecked, onChange, editItemList, setEditItemList }) {
  ...
  const { selectedValues, selecBoxHtml } = useSelectBox({
    ...
    pay_date: [{ placeholder: "선택" }, "전체 n위 달성 시"],
  ...
  useEffect(() => {
    ...
    if (findIndex !== -1) {
      setEditItemList(
        Object.values({
          ...editItemList,
          [findIndex]: { ...editItemList[findIndex], start_at: start_at, end_at: end_at, state: { 지급중: 1, 지급중지: 0 }[selectedValues.pay_step] },
        })
      );
    }
    ...
    <tr>
      <td className="check">
        <CheckBox
          ...
          name="check_box"
          onClick={e => {
            isChecked(e, addInput, setAddInput);
          }}
        />
      </td>
      <td className="num"></td>
      ...
      <td>
        <div className="select_input_wrap d-flex" name="state">
          {selecBoxHtml[2]}
        </div>
      </td>
