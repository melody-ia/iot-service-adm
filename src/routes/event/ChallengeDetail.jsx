...
export default function ChallengeAdd() {
  ...
  const copyUrl = () => {
    navigator.clipboard.writeText(serverUrl + "ChallengeWrite/" + challengeContents.ch_no);
  };
  ...
  const dataSubmit = async () => {
    const checkContents = ["ch_title", "ch_stamp_board", "ch_stamp_count", "ch_max_stamp", "ch_point_name", "ch_point_price", "stamp_to_point"];
    if (checkContents.some(el => !challengeContents[el])) return alert("입력항목을 모두 입력해 주세요.");
    let data = {};
    ...
    const res = await postData("challenge/update", { mb_no, start_at, end_at, ch_no, ...data });
    if (res.code === 200) {
      alert("수정되었습니다.");
      navigate("/ChallengeList");
    }
  ...
  if (challengeContents)
    return (
      <>
              ...
              <div className="wirte_area stamp_policy">
                ...
                <div className="flex_box">
                  <div className="input_ty02 flex_left">
                    <label htmlFor="">
                      글쓰기 URL
                      <img src={copy} className="icon click" onClick={copyUrl} />
                    </label>
                    <input
                      type="text"
                ...
                <div className="flex_box">
                  <div className="input_ty02 flex_left">
                    <label htmlFor="">
                      총 참여 회원 수 <img src={arrowRight} alt="" className="icon click" onClick={() => setModalOpen(true)} />
                    </label>
                    <input type="text" value={Number(challengeContents.sum_member).toLocaleString("ko-KR")} disabled />
                  </div>
                  <div className="input_ty02 flex_right">
                    <label htmlFor="">총 등록된 글 개수</label>
                    <input type="text" value={Number(challengeContents.sum_board).toLocaleString("ko-KR")} disabled />
                  </div>
                </div>
                <div className="flex_box">
                  <div className="input_ty02 flex_left">
                    <label htmlFor="">총 적립된 도장 개수</label>
                    <input type="text" value={Number(challengeContents.sum_stamp).toLocaleString("ko-KR")} disabled />
                  </div>
                  <div className="input_ty02 flex_right">
                    <label htmlFor="">총 지급된 포인트 금액</label>
                    <input type="text" value={Number(challengeContents.sum_point).toLocaleString("ko-KR")} disabled />
                  </div>
                </div>
              </div>
            </div>
        ...
        {modalOpen && (
          <>
            <ChallengeListDetailModal modalClose={() => setModalOpen(false)} ch_no={challengeContents.ch_no} />
